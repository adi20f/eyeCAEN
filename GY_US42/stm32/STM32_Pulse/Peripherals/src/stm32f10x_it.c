...
{   
 	if((TIM2CH1_CAPTURE_STA&0X80)==0)//CH1 还未成功捕获	
	{  		
		if (TIM_GetITStatus(TIM2, TIM_IT_CC1) != RESET)//捕获1发生捕获事件
		{	
			
			if(TIM2CH1_CAPTURE_STA&0X40)		//捕获到一个下降沿为真 		   发生上升沿中断后再次发生中断为下降沿
			{	  			
				TIM2CH1_CAPTURE_STA|=0X80;		//标记成功捕获到一次上升下降脉宽
				TIM2CH1_CAPTURE_DOWNVAL=TIM_GetCapture1(TIM2);
			   ...
			   tempup1=TIM2CH1_CAPTURE_DOWNVAL-TIM2CH1_CAPTURE_UPVAL+tim2_T;//得到总的高电平时间
			...
			else  								//发生捕获事件但不是下降沿，第一次捕获上升沿，清零，定时器开始计数
			{
				...
				TIM2CH1_CAPTURE_STA|=0X40;		//标记以捕获到了上升沿
		...
		if(i>3)//i等于4时，已经接收到数据量字节rebuf[3]
		{
			...
			switch(rebuf[2])//接收完毕后处理
			{
				case 0x45:
					...
					{
						memcpy(Sonar_data,rebuf,8);//拷贝接收到的数据
						Receive_ok=1;//接收完成标志
					}
					break;
